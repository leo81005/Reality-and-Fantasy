...
def add_ca_loss_per_attn_map_to_loss(loss, attn_map, object_number, bboxes, object_positions, use_ratio_based_loss=True, fg_top_p=0.2, bg_top_p=0.2, supp=10.0, fg_weight=1.0, bg_weight=1.0, fuse_bg=False, verbose=False):
    """
    ...
    for obj_idx in range(object_number):
        ...
        for obj_position in object_positions[obj_idx]:
            ...
            if use_ratio_based_loss:
                ...
                if fuse_bg:
                    obj_loss += ((ca_map_obj * (1 - mask_1d)).topk(k=k_bg).values.mean(dim=1)).sum(dim=0) * supp
                else:
...
def compute_ca_lossv3(saved_attn, bboxes, object_positions, guidance_attn_keys, ref_ca_saved_attns=None, ref_ca_last_token_only=True, ref_ca_word_token_only=False, word_token_indices=None, index=None, ref_ca_loss_weight=1.0, guid=15.0, verbose=False, **kwargs):
    """
    ...
    for attn_key in guidance_attn_keys:
        ...
        if ref_ca_saved_attns is not None:
            loss = add_ca_loss_per_attn_map_to_loss(loss, attn_map, object_number, bboxes, object_positions, fuse_bg=True, verbose=verbose, **kwargs)
        else:
    ...
    if ref_ca_saved_attns is not None:
        ...
        loss += ref_loss * guid / (object_number * num_attn)
